...
namespace SharpLoad.Core.Models
{
    public class LoadTestScript : BaseModel
    {
        ...
        public LoadTestScript(string name, Uri baseServerAddress, int maxSimultaneousClients, int spawnRate, int intervalBetweenRequests, TestStatus status, IEnumerable<Request> requests)
        {
            Name = name;
            SpawnRate = spawnRate;
            IntervalBetweenRequests = intervalBetweenRequests;
